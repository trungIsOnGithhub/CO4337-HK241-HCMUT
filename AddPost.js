...
const AddPost = () => {
  ...
  const addNewTagOnClick = useCallback(async (newTagLabel) => {
    let response = await apiCreateNewPostTag(newTagLabel);

    if (response && response.postTag) {
      ...
      <div className='p-4 '>
        <form onSubmit={() => {handleCreateBlogPost(); console.log('---')}}>
          ...
          <div className='w-full my-6 flex gap-4' style={{zIndex:88}}>
            ...
            />
            <Button
              className="px-4 p-3 rounded-md text-white bg-main font-semibold my-2"
