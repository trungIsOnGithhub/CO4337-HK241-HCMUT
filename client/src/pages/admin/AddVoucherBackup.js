...
const AddVoucher = () => {
  ...
  const handleSelectProductChange = useCallback(selectedOptions => {
    // Update selected services
...
{/* 
              ...
              <div className='w-full my-6 flex flex-col gap-4'>
                {selectedProduct.map((product) => (
                  <div key={product} className='w-[80%]'>
                    <label htmlFor={`percentageDiscount-${product}`} className='block mb-2 text-[#00143c] font-medium'>
                      Percentage Discount of {products.find(s => s._id === product)?.name}
                    </label>
                    <input
                      id={`percentageDiscount-${product}`}
                      type='number'
                      step='0.01'
                      min='0'
                      max='100'
                      className='w-full p-2 border border-gray-300 text-black outline-none'
                      placeholder='Percentage Discount ...'
                      onChange={handlePercentChange(product)}
                      value={percentageDiscount.find(item => item.id === product)?.value || ''}
                      required
                    />
                    {errors[`percentageDiscount-${product}`] && (
                      <small className="text-xs text-red-500">
                        {errors[`percentageDiscount-${product}`].message}
                      </small>
                    )}
                  </div>
                ))}
            ...
            {voucherType === 'fixed' && selectedProduct.length > 0 && (
              <div className='w-full my-6 flex flex-col gap-4'>
                {selectedProduct.map((product) => (
                  <div key={product.value} className='w-[80%] flex flex-col items-start text-[#00143c] font-medium'>
                    <label className='block mb-2'>{products.find(s => s._id === product)?.name}</label>
                    <div className='p-2 flex w-full justify-between items-center gap-16 text-[#00143c]'>
                      <Slider
                        defaultValue={0}
                        min={0}
                        ...
                        onChange={handleSliderChange(product)}
                        value={fixedAmount.find(item => item.id === product)?.value || 0}
                        sx={{
                          '& .MuiSlider-markLabel': {
                            color: '#00143c',
                            paddingLeft: '20px',
                      ...
                      <input
                        ...
                        className="border border-[#dee1e6] rounded p-1 w-32 ml-2 text-[#00143c] outline-none text-center bg-blue-200 "
                        placeholder="Enter amount"
                        onChange={(e) => handleFixedAmountInputChange(product, e.target.value)}
                        value={fixedAmount.find(item => item.id === product)?.value || ''}
                      />
                    </div>
                  </div>
                ))}
