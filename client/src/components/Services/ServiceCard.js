...
import defaultProviderImage from '../../assets/defaultProvider.jpg'
const ServiceCard = ({provider}) => {
  const navigate = useNavigate()
  return (
    <div 
      onClick={()=> navigate(`/detail_provider/${provider?._id}`)}
      className='w-1/3 px-[10px] mb-[20px] cursor-pointer'>

      <div className='border flex-auto flex w-full'>
        <img src={provider?.images[0] || defaultProviderImage} alt='products' className='max-w-[200px] max-h-[160px] object-cover p-4' />
